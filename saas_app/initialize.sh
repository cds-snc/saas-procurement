...
if [ "${social_account}" -eq 0 ]; then
    ...
    if [ "${social_app_model}" = "socialaccount.socialapp" ]; then
        ...
        jq '.[7].fields.client_id=env.SOCIAL_APPLICATION_CLIENT_ID | .[7].fields.secret=env.SOCIAL_APPLICATION_SECRET_KEY | .[4].fields.password = env.MANAGER_PASSWORD' fixtures/fixtures_initial_data.json >> tmp.json && mv tmp.json fixtures/fixtures_initial_data.json
